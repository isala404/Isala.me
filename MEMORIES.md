...
- An element with `data-umami-event` reaches Cairn once as `element.clicked` with that name as `label` and its `data-umami-event-*` extras as props; the generic `nav-<section>` / `external-link` / `contact-<type>` / `social-<platform>` labels are skipped for that click so nothing double-counts. Unnamed links still get the generic labels
